...
- **连接管理**：心跳保活、全局/单用户连接数上限、**按连接维度的三级背压**（暂停推送 → 排水重同步 → 断开慢连接）、优雅退出
...
npm test           # 24 个测试（16 个集成测试 + 8 个 Hub 背压单元测试）
```
...
├── hub.js      连接注册中心：房间索引、广播、未 ACK 追踪、连接级背压（暂停/排水恢复/超时断开）、心跳/重发扫描
...
背压状态（未 ACK 计数、发送缓冲区、暂停标记、投递游标）**全部挂在单条 `Connection`
上，与用户无关**：同一账号多台设备各自独立计量，任何一台慢设备被暂停或断开，都不会
波及该用户的其他设备连接，也不触碰共享的用户级确认游标（`cursors` 表只在客户端显式
ACK 时以 `MAX` 单调推进）。

每条连接有两个独立的高水位信号，满足其一即触发：

| 信号 | 配置 | 含义 |
|---|---|---|
| 未 ACK 消息数 | `MAX_UNACKED_PER_CONN`（默认 1000） | 客户端收到但迟迟不累积 ACK |
| 发送缓冲区字节数 | `MAX_SEND_BUFFER_BYTES`（默认 1 MiB） | 客户端读取缓慢，TCP 发送缓冲堆积（`ws.bufferedAmount`） |

处置分三级：

1. **暂停推送**：达到高水位后，该连接的实时 `msg` 帧一律跳过——**不发送、也不入未 ACK
   队列**（从源头保证内存有界），仅在连接上记录缺口房间；控制帧（`ack`/`sync_done`/
   `notice` 等）照常下发，房间内其他连接的广播完全不受影响。
2. **排水后重新同步**：客户端补 ACK、或缓冲区回落到「低水位」（高水位 ×
   `BACKPRESSURE_RESUME_RATIO`，默认 0.5，滞回防抖）后自动恢复；服务端按**该连接私有
   的投递游标**（本连接最后实际投递到的 seq，与用户游标独立）经现有 `sync` 机制回放暂停
   期间错过的消息，分批、可续拉、按 seq 幂等，不丢不重。回放途中若再次触顶则以实际投递
   水位截断本批（`sync_done.hasMore=true`），客户端续拉即可。
3. **断开连接**：暂停持续超过 `BACKPRESSURE_TIMEOUT_MS`（默认 30s）仍无法排水，判定
   客户端无消费能力，**仅 `terminate` 这一条连接**（慢读者往往连 close 帧都读不走，
   terminate 立即释放 socket 与该连接的全部积压内存），只从索引中摘除它自己；同用户其他
   设备不断。客户端重连后携带本地 `lastSeq` 走 `join`/`sync` 完整补发——游标机制不受
   任何破坏。

心跳判死与 ACK 最大重发次数的既有断连路径保持不变；连接暂停期间不叠加超时重发，
避免向已经堆满的缓冲区继续灌数据。
...
| `MAX_UNACKED_PER_CONN` | `1000` | 单连接未 ACK 积压高水位（达到即暂停该连接推送） |
| `MAX_SEND_BUFFER_BYTES` | `1048576` | 单连接发送缓冲区高水位（1 MiB，慢读者背压） |
| `BACKPRESSURE_RESUME_RATIO` | `0.5` | 低水位 = 高水位 × 该比例（滞回，防边界抖动） |
| `BACKPRESSURE_TIMEOUT_MS` | `30000` | 暂停宽限：超时仍未排水则仅断开该连接 |
| `BACKPRESSURE_CHECK_INTERVAL_MS` | `1000` | 背压扫描周期（检测缓冲堆积/暂停超时/排水恢复） |
